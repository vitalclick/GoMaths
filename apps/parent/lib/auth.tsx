...
const EXPIRES_KEY = "gomaths.parent.access.expires";
 ...
 * `TypeError: Network request failed`, which is meaningless to a parent on
...
/**
 * Authenticated fetch for the parent app. Refreshes the access token when
 ...
 * every session the parent has. Concurrent callers share one request.
 */
...
async function requestRefresh(): Promise<RefreshOutcome> {
  ...
  if (!res.ok) {
    ...
    // having a bad day and must not sign the parent out.
