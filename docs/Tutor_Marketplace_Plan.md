...
It is the *right* second business for GoMaths — but **only if Phase 1 proves the learning hypothesis**. The marketplace amplifies whatever the core product does. If the core product doesn't move outcomes, the marketplace amplifies nothing.
...
Most parents in SA wanting tutoring rely on word-of-mouth and unstructured WhatsApp networks. There is no dominant marketplace. A trusted, GoMaths-branded marketplace — *combined with* the AI tutor, *combined with* the curriculum content, *combined with* the school relationships built in Phase 1 — is a defensible position no global edtech currently occupies in SA.
